    ...
    def load_pretrained(self, ckpt=None, key="model"):
        ...
        try:
            ...
            if isinstance(_ckpt, dict) and key in _ckpt:
                ...
                state = _ckpt
            if state is None:
                raise ValueError(f"Unsupported checkpoint format keys: {list(_ckpt.keys()) if isinstance(_ckpt, dict) else type(_ckpt)}")
            incompatibleKeys = self.load_state_dict(state, strict=False)
